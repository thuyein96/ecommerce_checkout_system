...
describe("normalizePointsToRedeem", () => {
    test("snaps down to nearest multiple of 10 when within caps", () => {
        expect(normalizePointsToRedeem(135, 250, 37)).toBe(130);
    });

    test("caps by balance", () => {
        expect(normalizePointsToRedeem(400, 250, 37)).toBe(250);
    });

    test("caps by payable amount", () => {
        expect(normalizePointsToRedeem(999, 1000, 37)).toBe(370);
    });

    test("negative request returns 0", () => {
        expect(normalizePointsToRedeem(-50, 200, 100)).toBe(0);
    });

    test("zero request returns 0", () => {
        expect(normalizePointsToRedeem(0, 200, 100)).toBe(0);
    });

    test("uses smaller balance (already multiple of 10)", () => {
        expect(normalizePointsToRedeem(50, 40, 999)).toBe(40);
    });

    test("tiny payable caps redemption (snap to 30 pts for ฿3)", () => {
        expect(normalizePointsToRedeem(44, 1000, 3)).toBe(30);
    });
